...
def test_meteor_geodetic_coordinates_calculation():
    """Test the calculation of meteor geodetic coordinates"""
    ...
    vector_ondrejov = calculate_meteor_plane(meteor_ondrejov) + geodetic_to_geocentric(ondrejov)
    vector_kunzak = calculate_meteor_plane(meteor_kunzak) + geodetic_to_geocentric(kunzak)

    # Solve the intersection with the meteor plane
    for point in meteor_ondrejov:
        meteor_point_geocentric = calculate_meteor_point_vector(point, vector_ondrejov, vector_kunzak)

        # Convert to geodetic coordinates
        meteor_point = geocentric_to_geodetic(meteor_point_geocentric)
        print(f' <trkpt lat="{round(meteor_point[1], 5)}" lon="{round(meteor_point[0], 5)}"><ele>{round(meteor_point[2], 1)}</ele></trkpt>')

    print('\n')

    for point in meteor_kunzak:
        meteor_point_geocentric = calculate_meteor_point_vector(point, vector_kunzak, vector_ondrejov)
...
if __name__ == '__main__':
    test_meteor_geodetic_coordinates_calculation()
